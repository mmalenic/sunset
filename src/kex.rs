...
use ml_kem::{
    ...
    Key, Ciphertext, MlKem768, DecapsulationKey, KeyExport, Seed, B32,
};
use rand_core::OsRng;
use sha2::Sha256;
...
impl KexMlkemX25519 {
    ...
    const _CHECK0: () = assert!(
        Self::MLKEM768_PUBKEY_SIZE
            == size_of::<Key<EncapsulationKey<MlKem768>>>()
    );
    const _CHECK1: () = assert!(
        Self::MLKEM768_CIPHERTEXT_SIZE == size_of::<Ciphertext<MlKem768>>()
    );
